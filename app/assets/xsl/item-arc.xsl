...
<xsl:stylesheet version="1.0" xmlns:foxml="info:fedora/fedora-system:def/foxml#"
    ...
    <xsl:template match="/">
      <xsl:apply-templates select="foxml:digitalObject/foxml:datastream[@ID='ITEM-ARC-EN']"></xsl:apply-templates>
    </xsl:template>

	<xsl:template match="foxml:digitalObject/foxml:datastream[@ID='ITEM-ARC-EN']">
