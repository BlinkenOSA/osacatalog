...
module MarcHelper
            ...
            field.each do |subfield|
              unless ["?","="].include?(subfield.code)
                if subfield.code == "e"
                  ...
                  nil
                elsif subfield.code != "e" and subfield.code != "4" and subfield.code != "c"
                  link_text << "#{subfield.value} "
                elsif subfield.code == "c"
                  role_text << "#{subfield.value}"
                end
              end
            end
  ...
  def get_holdings(marc)
    ...
    marc.find_all{|f| ('952') === f.tag}.each do |field|
      ...
      text << content_tag(:td, field["a"] ? Constants::KOHA_COLLECTIONS[field["8"]] : "-")
  ...
  def get_film_library_call_number(marc)
    data = ""
    marc.find_all{|f| ("099") === f.tag}.each do |field|
      data << field["f"]
    end
  ...
  def link_to_author_from_marc(marc, opts={})
    if marc["100"]
      ...
      opts[:search_options] ||= {:controller => "catalog", :action => 'index', :search_field => 'creator_target'}
      link, extra, subs = [],[], []
      marc["100"].each do |sub_field|
        unless Constants::EXCLUDE_FIELDS.include?(sub_field.code)
          subs << sub_field.code
          if subs.include?("e") or subs.include?("4") or subs.include?("d")
      ...
      {:label  => opts[:label],
       :fields => [{:field      => [link_to(link.join(' '), opts[:search_options].merge(:q => "\"#{link.join(' ').chomp(',')}\"")), extra].flatten.compact.join(" ").html_safe,
